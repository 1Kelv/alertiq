...
const MODE_LABELS = { set1: 'Set 1', set2: 'Set 2', mixed: 'Mixed', exam: 'Exam', idv: 'ID&V', kyc: 'KYC', chargeback: 'Chargebacks' };
...
export default function TrainerDashboard({ onBack }) {
  ...
  const [tab, setTab]           = useState('sessions'); // 'sessions' | 'builder'
  ...
  const cohorts = ['all', ...Array.from(new Set(sessions.map(s => s.cohort).filter(Boolean)))];
    ...
    <div className="screen active">
      <div className="dashboard-wrap">
        ...
        <div className="dash-main-tabs">
          <button className={`dash-main-tab${tab === 'sessions' ? ' active' : ''}`} onClick={() => setTab('sessions')}>
            📊 Sessions
          </button>
          <button className={`dash-main-tab${tab === 'builder' ? ' active' : ''}`} onClick={() => setTab('builder')}>
            🛠 Alert Builder
          </button>
        </div>
        ...
        {tab === 'sessions' && (
          <>
            <div className="dash-stats">
              ...
              <div className="dash-stat-box"><div className="dsv">{avgPct}%</div><div className="dsl">Avg Score</div></div>
              <div className="dash-stat-box"><div className="dsv">{topScore}%</div><div className="dsl">Top Score</div></div>
              <div className="dash-stat-box"><div className="dsv">{passRate}%</div><div className="dsl">Pass Rate (≥75%)</div></div>
            </div>

            <div className="dash-controls">
              <div className="dash-filters" style={{ flexWrap: 'wrap', gap: 6 }}>
                {['all','set1','set2','mixed','exam','idv','kyc','chargeback'].map(f => (
                  <button key={f} className={`filter-btn${filter===f?' active':''}`} onClick={() => setFilter(f)}>
                    {f === 'all' ? 'All modules' : MODE_LABELS[f]}
                  </button>
                ))}
              </div>
            ...
            {cohorts.length > 1 && (
              <div className="dash-filters" style={{ marginBottom: 16, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px', alignSelf: 'center', marginRight: 4 }}>Cohort:</span>
                {cohorts.map(c => (
                  <button key={c} className={`filter-btn${cohortFilter===c?' active':''}`} onClick={() => setCohortFilter(c)}>
                    {c === 'all' ? 'All cohorts' : c}
                  </button>
                ))}
              </div>
                ...
                <table className="dash-table">
                  ...
                  <tbody>
                    {sorted.map(s => {
                      ...
                      const sc = String(s.time_taken%60).padStart(2,'0');
                      return (
                        <tr key={s.id}>
                          <td className="td-name">{s.student_name}</td>
                          <td style={{ fontSize: 12, color: 'var(--text3)' }}>{s.cohort || '—'}</td>
        ...
        {tab === 'builder' && <CustomAlertBuilder />}
      </div>
    </div>
  );
}
